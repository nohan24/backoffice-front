...
export default function Transmission (){
    ...
    useEffect(() => {
        getTransmissions().then((response) => {
                    ...
                    <form onSubmit={(e) => ajout(e)}>
                        <Stack direction="row" gap={1} sx={{width: '100%'}} alignItems={!error ? "flex-end" : "center"}>
                            <FormControl error={error}>
                                <FormLabel sx={{fontWeight: 600}}>Nom de la transmission</FormLabel>
                                <Input name="transmission" sx={{minWidth:'300px'}}/>
                                {
                            ...
                            <Button type="submit" startDecorator={<Add />}>Nouvelle transmission</Button>
                        </Stack>
                    </form>
                </Grid>
                <Grid xs={12}>
                    <Loading data={transmissions}>
                        <Box sx={{padding: '5px 0px'}}>
                            ...
                            >
                                ...
                                <tbody>
                                {
                                    transmissions ? transmissions.map((d,index) => {
                                        return(
                                            <tr id={index}>
                                                <td>
                                                    <Typography
